<head>
    <meta http-equiv="Content-Type" content="text/html; charset=gb2312">
    <style type="text/css">
        <!--
        body {
            margin: 0;
            background-color: #333333;
        }

        table {
            border: 0;
            margin: 0;
            border-collapse: collapse;
            border-spacing: 0;
        }

        table tr {
            height: 25px;
        }

        table tr td {
            padding: 0;
            text-align: center;
        }

        .Ours {
            background-color: #444444;
        }

        .Ours .High {
            color: #ffff00;
        }
        -->
    </style>
    <link href="/Models/Css/2008.css" rel="stylesheet" type="text/css">
</head>
<body onload="parent.document.getElementById('World').style.display='block';">
<table>
    <tbody>
    <tr class="Title">
        <td width="40">Rank</td>
        <td width="120">Name</td>
        <td width="30">Beg</td>
        <td width="40">Int</td>
        <td width="40">Exp</td>
        <td width="25">Sum</td>
    </tr>
    <tr>
        <td class="Bold">No. <span class="Signest">1</span></td>
        <td class="High">Kamil Muranski</td>
        <td class="Text Beg">0.49</td>
        <td class="Text Int">7.03</td>
        <td class="Text Exp">31.13</td>
        <td class="Text Signest">39</td>
    </tr>
    <tr class="Ours">
        <td class="Bold">No. <span class="Signest">2</span></td>
        <td class="High">Ze-En Ju</td>
        <td class="Text Beg">0.49</td>
        <td class="Text Int">7.76</td>
        <td class="Text Exp">31.62</td>
        <td class="Text Signest">40</td>
    </tr>
    <tr class="Ours">
        <td class="Bold">No. <span class="Signest">3</span></td>
        <td class="High">Wei-Jia Guo</td>
        <td class="Text Beg">0.54</td>
        <td class="Text Int">7.51</td>
        <td class="Text Exp">32.02</td>
        <td class="Text Signest">41</td>
    </tr>
    <tr class="Ours">
        <td class="Bold">No. <span class="Signest">4</span></td>
        <td class="High">Xian-Yao Zhang</td>
        <td class="Text Beg">0.65</td>
        <td class="Text Int">8.33</td>
        <td class="Text Exp">32.62</td>
        <td class="Text Signest">42</td>
    </tr>
    <tr class="Ours">
        <td class="Bold">No. <span class="Signest">5</span></td>
        <td class="High">Dan Zhou</td>
        <td class="Text Beg">0.54</td>
        <td class="Text Int">9.19</td>
        <td class="Text Exp">31.90</td>
        <td class="Text Signest">42</td>
    </tr>
    <tr>
        <td class="Bold">No. <span class="Signest">6</span></td>
        <td class="High">Mao Igarashi</td>
        <td class="Text Beg">0.58</td>
        <td class="Text Int">9.06</td>
        <td class="Text Exp">32.59</td>
        <td class="Text Signest">43</td>
    </tr>
    <tr>
        <td class="Bold">No. <span class="Signest">7</span></td>
        <td class="High">Ian Fraser</td>
        <td class="Text Beg">0.60</td>
        <td class="Text Int">9.45</td>
        <td class="Text Exp">32.95</td>
        <td class="Text Signest">43</td>
    </tr>
    <tr>
        <td class="Bold">No. <span class="Signest">8</span></td>
        <td class="High">Pavel Mishin</td>
        <td class="Text Beg">0.76</td>
        <td class="Text Int">8.41</td>
        <td class="Text Exp">34.15</td>
        <td class="Text Signest">44</td>
    </tr>
    <tr>
        <td class="Bold">No. <span class="Signest">9</span></td>
        <td class="High">Yu Suzuki</td>
        <td class="Text Beg">0.74</td>
        <td class="Text Int">9.77</td>
        <td class="Text Exp">33.27</td>
        <td class="Text Signest">44</td>
    </tr>
    <tr class="Ours">
        <td class="Bold">No. <span class="Signest">10</span></td>
        <td class="High">Yao-Yu Zhu</td>
        <td class="Text Beg">0.75</td>
        <td class="Text Int">8.36</td>
        <td class="Text Exp">34.68</td>
        <td class="Text Signest">44</td>
    </tr>
    <tr align="right">
        <td colspan="2" class="Text" height="35" align="left"><a href="http://www.minesweeper.info/worldranking.html" target="_blank" class="Sign">点击查看完整世界排行</a></td>
        <td colspan="4" class="Text" height="35"><p>更新时间：<span class="Title">2020.7.19</span></p></td>
    </tr>
    </tbody>
</table>
</body>