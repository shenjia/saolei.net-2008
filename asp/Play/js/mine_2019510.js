...
Container.prototype.init=function(level){
	...
	if(exist!=null){
		...
		if(level==3){//高级
			...
			window.parent.document.getElementById('Window_Border').style.top=window.parent.document.body.scrollTop+(clientHeight-376)/2;
			this.rows=16;
			this.columns=30;
			this.bombNumber=99;
			this.level=3;
			...
			document.getElementById("video_control").style.left='4px';
			//当前所在frame居中
			window.parent.document.getElementById('Window_Border').style.left=(window.parent.document.body.clientWidth-418)/2;
			window.parent.document.getElementById('Window_Border').style.top=window.parent.document.body.scrollTop+(clientHeight-376)/2;
			this.rows=16;
			this.columns=16;
			this.bombNumber=40;
			...
			document.getElementById("video_control").style.left='4px';
			//当前所在frame居中
			window.parent.document.getElementById('Window_Border').style.left=(window.parent.document.body.clientWidth-290)/2;
			window.parent.document.getElementById('Window_Border').style.top=window.parent.document.body.scrollTop+(clientHeight-357)/2;
			this.rows=8;
			this.columns=8;
			this.bombNumber=10;
			this.level=1;
		...
		//当前frame宽高
		window.parent.document.getElementById('Window_Video').width=document.getElementById('border').offsetWidth;
		window.parent.document.getElementById('Window_Video').height=document.getElementById('border').offsetHeight;

		//加载完成重新显示frame
		window.parent.document.getElementById('Window_Border').style.display='block';//避免浏览器前进后退重新显示
		// window.parent.document.getElementById('Window_Video').style.display='block';
	}else{
		//首次加载(扫雷网主界面)时不显示录像iframe
